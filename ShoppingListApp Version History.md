...
## v1.4.15 Reward Lottery Session Logic

### Completed
- 買い物モードの報酬抽選タイミングを、全商品未チェック状態から1つ目をチェックした時点へ変更
- 報酬結果を、すべての商品チェックが外されるまで保持する仕様へ変更
- 最後の商品チェックをON/OFFしても同じ報酬パターンを再表示できるよう調整
- 既存の5分間再抽選されない制御を削除
- 商品数とフリーメモ数の合計が3件以下の場合は必ずNormalになるよう制御
- 商品数とフリーメモ数の合計が4件以上の場合のみRare / Super Rare抽選対象に変更

## v1.4.16 Countdown Reward Effect

### Completed
- 買い物モードで未購入残数が3 / 2 / 1になった時にカウントダウン画像を表示する演出を追加
- 商品数とフリーメモ数の合計が4件以上の場合のみカウントダウン演出を実行
- カウントダウン画像を1.1秒表示後に自動で非表示にする仕様を追加
- 報酬結果パターンに応じて、カウントダウン画像のNormal / Rare / Super Rare見た目を抽選する仕様を追加
- 3 → 2 → 1 の順に抽選し、Rare以上が出た場合は以降の数字がNormalに戻らない昇格ルールを追加
- 全商品未チェック状態から1つ目をチェックした時点でカウントダウン見た目を決定し、全商品未チェックに戻るまで保持する仕様を追加
- チェックを外して再度残数が3 / 2 / 1になった場合、同じカウントダウン画像を再表示する仕様を追加
- APP_VERSION を 1.4.16 に更新
- service-worker cache version を更新

## v1.4.17 Product List Column Setting

### Completed
- ハンバーガーメニューの表示設定に、商品リストの1列 / 2列表示切替を追加
- 商品リストのデフォルト表示を2列表示に設定
- 対象選択モードと買い物モードの商品一覧で、1列表示と2列表示を切り替えられるように対応
- フリーメモ行は表示切替の対象外とし、従来どおり1列表示を維持
- 表示設定を保存し、再読み込み後も維持されるよう対応
- 既存バックアップとの互換性を維持
- APP_VERSION を 1.4.17 に更新
- service-worker cache version を更新

## v1.4.18 Generic Checklist Header Update

### Completed
- アプリの表示名を「買い物リストアプリ」から「チェックリスト」へ変更
- ブラウザタイトル、PWA表示名、iOSホーム画面表示名を汎用的な表現へ更新
- ヘッダ固定表示のアプリ名を廃止
- メイン画面のヘッダに、ハンバーガーメニュー・準備モード・実行モード・ヘルプボタンを配置
- 対象選択モードを「準備モード」、買い物モードを「実行モード」へ文言変更
- 準備モードでは、メモリボタン・全選択・全解除・フリーメモ追加を操作パネル内に維持
- 実行モードでは、本文側の操作パネルを廃止し、項目リストを直接表示するよう調整
- ツールチップ文言を「項目」「実行対象」中心の汎用表現へ更新
- ハンバーガーメニューの「買い物リスト」を「項目リスト」へ変更
- メイン導線・操作系文言の「商品」を「項目」中心へ整理
- 表示設定の「商品リストを2列表示」を「項目リストを2列表示」へ変更
- カテゴリ使用数表記を「〇件の項目で使用中」へ変更
- 一括追加・削除ダイアログなど操作文言を「項目」へ統一
- 「商品管理」を「項目管理」へ変更
- 管理画面タイトル「商品一覧」を「項目一覧」へ変更
- UI主要導線の文言を「項目」で統一
- 項目一括追加の入力例を新カテゴリ運用に合わせて更新
- カテゴリ個別追加の入力例を「例：買い物」へ更新
- GitHubリポジトリ名、フォルダ名、GitHub Pages URLは変更せず維持
- APP_VERSION を 1.4.18 に更新
- service-worker cache version を更新

## v1.4.19 Empty State Sample Registration

### Completed
- 項目管理の空状態に「サンプル項目を登録する」ボタンを追加
- カテゴリ管理の空状態に「おすすめカテゴリを登録する」ボタンを追加
- 準備モード / 実行モードの空状態にも「サンプル項目を登録する」ボタンを追加
- カテゴリ未登録時に「買い物」「持ち物」「タスク」をワンタップ登録できるよう対応
- 項目未登録時に買い物カテゴリのサンプル項目20件をワンタップ登録できるよう対応
- サンプル項目登録時に「買い物」カテゴリが存在しない場合は自動作成するよう対応
- 登録後に登録件数が分かる成功メッセージを表示
- 空状態時のみサンプル登録ボタンを表示し、既存データがある場合は非表示にするよう制御
- 重複登録防止の安全制御を追加
- APP_VERSION を 1.4.19 に更新
- service-worker cache version を更新

## v1.4.20 Startup Performance Patch

### Completed
- 起動時の不要な画面描画を削減し、初回表示を高速化
- 初期表示では、ユーザーに見えている画面のみ描画するよう最適化
- 非表示画面（項目管理 / カテゴリ管理）の描画を遅延実行するよう改善
- 起動時のヘッダ重複描画を削減
- Service Worker 登録を初回描画後へ遅延し、起動体感速度を改善
- 起動時のPWAスプラッシュ表示時間短縮を目的とした内部最適化を実施
- 既存機能・既存データ互換性は維持
- APP_VERSION を 1.4.20 に更新
- service-worker cache version を更新

## v1.4.21 Preparation Mode Category Filter

### Completed
- 準備モード限定でカテゴリ絞り込み機能を追加
- 準備モード操作パネル内に「カテゴリ：すべて」プルダウンを追加
- カテゴリ変更時に項目一覧へ即時フィルタ反映するよう対応
- 「未分類」項目を絞り込み対象に追加
- 実行モード / 項目管理 / カテゴリ管理には影響しないよう分離
- 画面移動後に準備モードへ戻った際、フィルタを「すべて」に自動リセット
- 全選択 / 全解除を表示中項目のみに適用するよう改善
- ツールチップ文言をフィルタ仕様に合わせて更新
...
## v1.4.23 Onboarding Guide Release

### Completed
- 初回利用者向けの4枚構成オンボーディングを追加
- 初回かつ項目0件の場合のみ自動表示するよう対応
- ハンバーガーメニューに「使い方を見る」を追加し、いつでも再表示できるよう対応
- オンボーディング画像 onboarding1.webp ～ onboarding4.webp を使用
- スライド形式でアプリ用途、準備モード、実行モード、買い物以外の使い方を案内
- 4枚目CTAを項目数に応じて切り替える仕様を追加
- 項目0件の場合は「サンプルデータを入れる」から既存サンプル登録処理へ接続
- 項目登録済みの場合は「閉じる」表示とし、既存データへ影響しないよう制御
- 4枚目に「あとで自分で登録する」リンクを追加
- localStorage にオンボーディング完了状態を保存
- APP_VERSION を 1.4.23 に更新
- service-worker cache version を更新

### Notes
- 既存データ構造の変更なし
- localStorage既存キーとの互換性維持
- 既存のサンプル登録処理を再利用
- Reward / Countdown / メモリ機能への影響なし

## v1.4.24 Onboarding Cleanup and Image Refresh

### Completed
- オンボーディング関連コードを整理
- 不要になったタイトル・説明文レンダリングと関連CSSを削除
- オンボーディング画像定義をシンプル化
- 4枚目オンボーディング画像を更新
- APP_VERSION を 1.4.24 に更新
- service-worker cache version を更新
...
## v1.4.25 Onboarding Visual Final Adjustments

### Completed
- オンボーディング画像4枚を1080x1920版へ更新
- スマホ表示時の文字サイズ・視認性を改善
- Slide1〜3の画像表示バランスを調整
- Slide4 CTAボタン位置を最終調整
- Slide4 補助リンク位置を最終調整
- スライド間のカード高さズレを改善
- オンボーディング全体の視覚品質を向上
...
## v1.4.26 Onboarding Width Optimization

### Completed
- オンボーディング画像の左右余白を削減
- 画像表示幅を拡大しスマホ画面で見やすく調整
- 白カード内の表示バランスを改善
- CTA / ドット / 補助リンク位置を微調整
- オンボーディング全体の視認性を向上

### Technical
- APP_VERSION を 1.4.26 に更新
- 必要に応じて service-worker キャッシュ更新
...
## v1.4.27 Sample Data Update

### Completed
- サンプル項目を18件に見直し
- サンプルカテゴリを「買い物」「持ち物」「毎日の習慣」に更新
- 買い物以外の用途が伝わるサンプルデータへ変更
- 「持ち物」カテゴリのサンプル項目を追加
- 「毎日の習慣」カテゴリのサンプル項目を追加
- サンプル登録時に各項目のカテゴリを自動作成 / 再利用するよう調整
- サンプル登録完了トーストを「18件のサンプル項目を登録しました」に更新

### Notes
- 既存データがある場合はサンプル登録しない安全制御を維持
- 重複カテゴリを作成しない仕様を維持
- localStorage構造変更なし
- 既存データ互換性維持

## v1.4.28 Preparation Filter UI Polish

### Completed
- 準備モードのカテゴリフィルタUIを調整
- 「カテゴリ：」ラベルを削除し、画面上のノイズを削減
- カテゴリプルダウンの左端をフリーメモ入力欄と揃えるよう調整
- プルダウン表示文言を「すべて」から「すべてのカテゴリ」に変更
- カテゴリ一括追加の入力例を「毎日の習慣」に合わせて更新
...
## v1.4.29 Three Column Display Mode

### Completed
- 項目リストに3列表示モードを追加
- 表示設定を1列 / 2列 / 3列の切り替えUIへ更新
- 3列表示時の項目をジグザグ順で視線移動しやすく表示
- 3列表示時のみ行間にSVG矢印ガイドを追加
- 最終行が1件または2件の場合も進行方向に合わせて配置するよう調整
- 3列表示向けに文字サイズ・2行省略・カード余白を最適化
- displayColumns 設定を localStorage に保存するよう対応
- 既存の2列表示設定との互換変換を追加
- service-worker キャッシュに3列ガイド用SVGを追加

### Notes
- 既定表示は2列のまま維持
- 1列 / 2列表示の既存挙動変更なし
- sortOrder・保存順の変更なし
- localStorage既存データ互換性維持

## v1.4.30 Three Column Height Optimization

### Completed
- 3列表示専用のカード高さを微調整
- 上下paddingを最適化し一覧性を向上
- 3列表示時の文字line-heightを微調整
- 情報密度を高めつつ押しやすさを維持
